...
const showDogs = async (req, res) => {
  ...
  for (let i = 0; i < dogsArr.length; i++) {
    const newName = dogsArr[i].name;
    const newBreed = dogsArr[i].breed;
    const newAge = dogsArr[i].age;
    allDogsArr.push({ name: newName, breed: newBreed, age: newAge });
  }
  // Otherwise, we got a result and will send it back to the user.
  return res.json(allDogsArr);
};
